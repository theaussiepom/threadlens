...
- [ ] Version is `0.1.1` in `pyproject.toml`, `threadlens/__init__.py`, and `/api/v1/version`
...
3. GitHub Actions workflow `.github/workflows/publish-container.yml` publishes:
   ...
   - `ghcr.io/theaussiepom/threadlens:v0.1.1`
   - `ghcr.io/theaussiepom/threadlens:latest`

4. Create a GitHub Release from tag `v0.1.1` using notes from `CHANGELOG.md`.
...
Do not use manual `docker build` + `docker push` for releases.
